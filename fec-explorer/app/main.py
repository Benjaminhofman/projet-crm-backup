...
@app.get("/api/clients/filters", summary="Toutes les valeurs de filtres en une seule requête")
def get_clients_filters():
    conn = _get_db_conn()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def get_clients_distinct(field: str):
    ALLOWED = {"collaborateur", "assistant", "structure", "activite_r", "annee", "mois_cloture", "nom_client"}
    if field not in ALLOWED:
        raise HTTPException(status_code=400, detail=f"Champ non autorisé : {field}")
    conn = _get_db_conn()
    try:
        with conn.cursor() as cur:
            if field == "mois_cloture":
                cur.execute(
                    "SELECT DISTINCT SUBSTRING(date_de_cloture::text, 6, 2) "
                    "FROM clients WHERE date_de_cloture IS NOT NULL ORDER BY 1"
                )
            else:
                cur.execute(
